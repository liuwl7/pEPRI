...
while(my $frag_a=<SM>){
	if($frag_a=~/^@/){
		...
		if($strand_a eq "Plus"){
			print OUTA $chr_a,"\t",$loci_a-1,"\t",$loci_a+$match_a-1,"\t",$sample_a,"_",$id_a_info[0],"_",$id_a_info[1],"\t",255,"\t+\n";
		}
		...
		if($strand_b eq "Plus"){
			print OUTB $chr_b,"\t",$loci_b-1,"\t",$loci_b+$match_b-1,"\t",$sample_a,"_",$id_b_info[0],"_",$id_b_info[1],"\t",255,"\t+\n";
		}
